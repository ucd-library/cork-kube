...
async function apply(templateDir, opts, corkKubeConfig) {
  ...
  if( opts.namespace === '_file_default_' ) {
    log(` - Namespace: ${colors.yellow('file default')}`);
  } else if( opts.namespace ) {
    log(` - Namespace Override: ${colors.yellow(opts.namespace)}`);
  }
  ...
  let applyOpts = opts.namespace ? {...corkKubeConfig, namespace: opts.namespace} : corkKubeConfig;
  for( let template of templates ) {
    let output = await kubectl.apply(template, { isJson: true, stdin: true}, applyOpts);
    if( output.stderr ) {
