...
class BuildDependencies {
  ...
  async load(opts={}) {
    ...
    if( this.config.dependenciesDir ) {
      ...
      if( !fs.existsSync(dependenciesDir) ) {
        console.log(`Cloning build dependencies from ${this.REG_REPO.URL}`);
        exec(`git clone ${this.REG_REPO.URL} ${dependenciesDir}`);
      }
    }

    if( opts.pull ) {
      await this.pullRegistry();
    }
  ...
  async pullRegistry() {
    await exec(`git -C ${dependenciesDir} pull`);
    await exec(`git -C ${dependenciesDir} checkout main`);
  }
