...
class Build {
  ...
  async getBaseDockerBuildCmd(opts={}) {
    let cmd = ['docker buildx build --cache-to=type=inline,mode=max'];
    if( opts.cache === false ) {
      cmd.push('--no-cache');
    }

    // check if a local dev registry is set for the project
    let pushLocalDev = config.data.build.pushLocalDev;
    if( pushLocalDev ) {
      let gitInfo = await buildDependencies.gitInfo(config.localDir);
      pushLocalDev = Object.keys(config.data.build.localDevRegistry).find(k => {
        return gitInfo.httpRemote.includes(k);
      }) ? true : false;
    }

    let pushFlagSet = false;
    if( opts.production || pushLocalDev ) {
      cmd.push('--pull');
      if( opts.push !== false || pushLocalDev) {
        pushFlagSet = true;
        cmd.push('--push');
      }
    }
  ...
  async exec(opts={}) {
    ...
    let printSecretsInfo = {};
    for( let image of imageOrder ) {
      if( printSecretsInfo[image.project.name] ) continue;
      if( !image.project.secrets ) continue;
      for( let secret of image.project.secrets ) {
        printSecretsInfo[this._secretToEnvName(secret.name)] = secret.project+'/'+secret.name+':'+secret.version;
      }
    }
    if( Object.keys(printSecretsInfo).length ) {
      console.log(`\nSecrets Available:`);
      for( let secret in printSecretsInfo ) {
        console.log(`  ${secret}: ${printSecretsInfo[secret]}`);
      }
    }
    ...
    for( let image of imageOrder ) {
      ...
      envVars[this._secretToEnvName('image_'+image.project.name+'_'+image.name)] = image.tag;
    }
    ...
    if( !opts.dryRun ) {
      let first = true;
      for( let image of imageOrder ) {
        if( secrets[image.project.name] ) continue;
        if( !image.project.secrets ) continue;
        secrets[image.project.name] = {};
        ...
        for( let secret of image.project.secrets ) {
          ...
          console.log(`Loading secret ${secret.name}:${secret.version} from ${secret.project}`);
          let secretValue = await gcloud.getSecret(secret.name, {
            project : secret.project,
            version : secret.version,
          });
          secrets[this._secretToEnvName(secret.name)] = secretValue;
        }
      }
      if( !first ) console.log();
    }
    let env = Object.assign({}, secrets, process.env);
    ...
    for( let image of imageOrder ) {
      ...
      try {
        ...
        await exec(image.buildCmd, {env}, {output: 'realtime'});
      } catch(e) {
    ...
    if( opts.setEnv ) {
      console.log(`\nSetting environment variables in ${opts.setEnv}`);
      let env = new EnvFile(opts.setEnv);
  ...
  async setEnv(opts={}) {
    ...
    console.log(`\nImages to set:`);
    for( let image of imageOrder ) {
      console.log(`  ${this._secretToEnvName('image_'+image.project.name+'_'+image.name)}: ${image.tag}`);
      envVars[this._secretToEnvName('image_'+image.project.name+'_'+image.name)] = image.tag;
    }
...
${image.user ? 'USER '+image.user : ''}`;
  ...
  async _getBuildCmd(item, imageName, opts={}) {
    ...
    let buildCmd = [(await this.getBaseDockerBuildCmd(opts)).join(' ')];
  ...
  _getRegistry(item, opts={}) {
    ...
    if( config.data.build.localDevRegistry ) {
      let url = item.url || item.gitInfo.httpRemote;
      let key = Object.keys(config.data.build.localDevRegistry).find(h => url.includes(h));
      if( key ) {
        return config.data.build.localDevRegistry[key];
      }
    }
  ...
  async _loadBuildConfig(item, opts={}, renderTemplateVars=true) {
...
Dockerfile`;
  ...
  _getTemplateVars(item, opts={}) {
    ...
    if( item.secrets ) {
      for( let secret in item.secrets ) {
        let secretName = item.secrets[secret].name;
        repoImages['secret.'+secretName] = '${'+this._secretToEnvName(secretName)+'}';
      }
    }
