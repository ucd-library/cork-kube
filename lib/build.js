...
${image.user ? 'USER '+image.user : ''}`;
  ...
  async clone(url, version, opts={}) {
    if( !opts.cloned ) opts.cloned = new Set();
    if( opts.cloned.has(url) ) return opts.dir;
    opts.cloned.add(url);
    ...
    if( !opts.dir ) {
      ...
      //   fs.rmSync(opts.dir, {force: true, recursive: true});
      // }
    ...
    await buildDependencies.pullRepository(opts.dir, url, version);
    return opts.dir;
  }
