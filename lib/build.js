...
${image.user ? 'USER '+image.user : ''}`;
  ...
  async _getBuildCmd(item, imageName, opts={}) {
    ...
    if( !image.dockerfile && item.src ) {
      ...
      // TODO move this
      // fs.writeFileSync(image.dockerfile, item.src.dockerfile);
      // fs.writeFileSync(image.dockerignore, item.src.dockerignore);
    }
  ...
  _getRegistry(item, opts={}) {
    if( opts.useRegistry && opts.useRegistry.includes(item.name) ) {
      return item.buildConfig.registry;
    }
...
`;
  ...
  async googleCloudBuild(project, version, opts={}) {
    ...
    --config=${dir} \\
    --project=${gcbProject} \\
    --substitutions=_PROJECT=${project},_VERSION=${version},_REGISTRY=${buildDependencies.config.dependenciesUrl || ''},_USE_CACHE=${opts.cache}`;
