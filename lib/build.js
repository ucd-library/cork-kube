...
${cmd}`);
    ...
    for( let item of order ) {
      if( item.buildConfig ) {
        ...
        for( let imageName in item.buildConfig.images ) {
          ...
          buildCmd.push(`--tag ${registry}/${imageName}:${item.gitInfo.tag || item.gitInfo.branch}`);
  ...
  _loadBuildConfig(item, opts={}, renderTemplateVars=true) {
    ...
    if( !fs.existsSync(item.buildScript) ) {
      console.error(`No build script (${item.buildScript}) found for ${item.name}`);
      process.exit(1);
    }
  ...
  _getTemplateVars(item, opts={}) {
    ...
    for( let repo in item.buildConfig.repositories ) {
      ...
      for( let imageName in repoDep.buildConfig.images ) {
        repoImages[repo+'.'+imageName] = registry+'/'+imageName+':'+(repoDep.gitInfo.tag || repoDep.gitInfo.branch);
      }
    }
