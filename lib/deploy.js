...
class Deploy {
  ...
  async all(opts={}) {
    ...
    for( let service of c.services ) {
      await this.service(service.name, opts);
    }
  }

  async service(serviceName, opts={}) {
    if( typeof opts !== 'object' ) {
      opts = {debug: opts};
    }

    let corkKubeConfig = config.corkKubeConfig;
    let c = await this.renderTemplate(serviceName, opts);
    if( c.ignore ) {
  ...
  async removeSecret(secret, currentSecrets, currentConfigMaps) {
    if( typeof secret === 'string' ) {
      secret = {
        name: secret,
        namespace: kubectl.runtimeParams?.namespace || 'default',
      }
    }
    ...
    if( !currentSecrets ) {
      currentSecrets = await kubectl.getSecrets({namespace: 'ALL'});
    } 
    if( !currentConfigMaps ) {
      currentConfigMaps = await kubectl.getConfigMaps(corkKubeConfig);
    }
  
    if( currentSecrets.find(s => s.name === secret.name && s.namespace === secret.namespace) ) {
      console.log(colors.yellow(`Removing secret ${secret.namespace}.${secret.name}`));
      await kubectl.delete('secret', secret.name, {namespace: secret.namespace});
    } else if( currentConfigMaps.includes(secret.name) ) {
      console.log(colors.yellow(`Removing configMap ${secret.name}`));
      await kubectl.delete('configmap', secret.name, corkKubeConfig);
    } else {
      console.log(colors.yellow(`Secret ${secret.namespace}.${secret.name} not found, skipping removal`));
    }
  }
  ...
  async secrets() {
    ...
    let secrets = c?.secrets?.[corkKubeConfig?.env];
    let env = corkKubeConfig?.env;``
    if( !secrets ) {
      if( corkKubeConfig.alias && c?.secrets?.[corkKubeConfig.alias] ) {
        console.log(`Using alias ${corkKubeConfig.alias} for environment secrets`);
        secrets = c.secrets[corkKubeConfig.alias];
        env = corkKubeConfig.alias;
      } else {
        console.error(`Secrets not found for environment ${corkKubeConfig.env}`);
        process.exit(1);
      }
    }

    let currentSecrets = await kubectl.getSecrets({namespace: 'ALL'});
    let currentConfigMaps = await kubectl.getConfigMaps(corkKubeConfig);

    for( let secret of secrets ) {
      secret = {
        name: secret.k8sName,
        namespace: secret.namespace || kubectl.runtimeParams?.namespace || 'default', 
      }

      await this.removeSecret(secret, currentSecrets, currentConfigMaps);
      await this.secret(secret.name, env);
    }
  }
  ...
  async secret(secretName, env) {
    ...
    if( secret.fromEnvFile === true ) {
      console.log(` - Adding env file secret ${colors.green(secret.k8sName)} from gcsm secret ${colors.green(secret.gcsmName)}`);
      let contents = await gcloud.getSecret(secret.gcsmName, corkKubeConfig);
      await kubectl.createEnvFileSecret(secret.k8sName, contents, corkKubeConfig);
      return;
    } else if( secret.tls ) {
      console.log(` - Adding tls secret ${secret.namespace ? secret.namespace+'.' : ''}${colors.green(secret.k8sName)} from gcsm secrets cert=${colors.green(secret.certGcsmName)} and key=${colors.green(secret.keyGcsmName)}`);
      let cert = await gcloud.getSecret(secret.certGcsmName, corkKubeConfig);
      let key = await gcloud.getSecret(secret.keyGcsmName, corkKubeConfig);
      await kubectl.createTlsSecret(secret.k8sName, key, cert, secret.namespace);
      return;
    }
  ...
  async remove(serviceName) {
    let c = await this.renderTemplate(serviceName);
    let corkKubeConfig = config.corkKubeConfig;
  ...
  async renderTemplate(serviceName, opts={}) {
    let c = config.data.local;
    ...
    let env = config.corkKubeConfig?.env;
    let alias = config.corkKubeConfig?.alias;

    let envConfig = service?.environments?.[env];
    if( !envConfig && alias && service?.environments?.[alias] ) {
      console.log(`Using alias ${colors.yellow(alias)} for service ${colors.yellow(serviceName)} environment, ${colors.yellow(env)} not found`);
      envConfig = service?.environments?.[alias];
    }
    if( !envConfig ) envConfig = {};

    if( !envConfig.overlay ) {
      if( alias ) {
        console.log(`No overlay specified in ${colors.yellow(env)} found for service ${colors.yellow(serviceName)}. Using ${colors.yellow(`overlay=${env},${alias}`)}`);
        envConfig.overlay = [env, alias];
      } else {
    ...
    if( c.namespace && c.namespace.length ) {
      result.namespace = c.namespace[0];
    }
