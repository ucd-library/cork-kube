...
class Deploy {
  ...
  async renderTemplate(serviceName, env) {
    let c = config.data.local;
    ...
    if( env && !envConfig ) {
      console.warn(`Environment ${env} not found for service ${service}. Ignoring`);
    }
    ...
    return new Promise((resolve, reject) => {
      exec(contents, 
        ...
        (error, stdout, stderr) => {
          ...
          try {
            resolve(JSON.parse(stdout.split(this.LOAD_DELIMINATOR)[1]));
          } catch(e) {
            reject(e);
          } 
        }
