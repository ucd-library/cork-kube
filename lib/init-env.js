...
async function init(env, opts, activate=false) {
  ...
  if( corkKubeConfig.alias ) {
    if( !environments[corkKubeConfig.alias] ) {
      console.error(`Alias config ${corkKubeConfig.alias} not found in config: ${config.localFile}`);
      process.exit(1);
    }
    let aliasCorkKubeConfig = environments[corkKubeConfig.alias];
