...
class KubectlWrapper {
  ...
  async getCurrentUser() {
    try {
      ...
      let {stdout} = await exec(`kubectl auth whoami --request-timeout=5s -o=jsonpath="{.status.userInfo.username}"`);
      return stdout.trim();
    } catch(e) {
      return '';
    }
  ...
  async restart(type, name) {
    return exec(`kubectl rollout restart ${type} ${name}`);
  }
  ...
  createSecret(name, files, opts={}) {
    ...
    console.log(`kubectl create secret generic ${name} ${fileStr}`)
