...
EOF`);
  ...
  async exec(command, args={}, options) {
    ...
    // JM - commenting this out as some kubectl commands output warnings to stderr
    // Specifically: Warning: spec.SessionAffinity is ignored for headless services
    // Need to revisit later if this causes issues.
    // if( stderr ) {
  ...
  async renderKustomizeTemplates(template, overlay=null) {
    ...
    if( !templatePath ) {
      ...
      if( fs.existsSync(templatePath) ) {
        usedOverlay = 'base';
      } else if( fs.existsSync(path.join(template, 'kustomization.yaml') || fs.existsSync(path.join(template, 'kustomization.yml')) ) ) {
  ...
  getContextNsFlags(contextOnly=false, namespace=null) {
    ...
    if( namespace && contextOnly === false ) {
      ...
      } else if( namespace !== '_file_default_' ) {
        flags.push(`--namespace=${namespace}`);
      }
    }
    return flags.join(' ');
  }
