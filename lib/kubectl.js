...
class KubectlWrapper {
  ...
  async getContexts() {
    let {stdout} = await exec(`kubectl config view -o jsonpath="{.contexts[*].name}"`);
  ...
  async getNamespace(opts={}) {
    let cnsFlags = this.getContextNsFlags(opts, true);
  ...
  async getSecrets(opts={}) {
    let cnsFlags = this.getContextNsFlags(opts);
    let {stdout} = await exec(`kubectl get secrets ${cnsFlags} -o jsonpath="{.items[*].metadata.name}"`);
    return stdout.split(' ').map(n => n.trim());
  }
  ...
  createNamespace(namespace, opts={}) {
    let cFlag = this.getContextNsFlags(opts, true);
  ...
  getContextNsFlags(opts={}, contextOnly=false) {
    let flags = [];
    if( opts.context ) {
      flags.push(`--context=${opts.context}`);
    }
    if( opts.namespace && contextOnly === false ) {
      flags.push(`--namespace=${opts.namespace}`);
    }
    return flags.join(' ');
  }
