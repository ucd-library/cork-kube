...
class TemplateConfig {
  ...
  async getVariables(opts) {
    let localDevRegistry = null;
    if( config?.data?.build?.localDevRegistry && config?.corkKubeConfig?.gitInfo?.httpRemote ) {
      let match = Object.keys(config.data.build.localDevRegistry).find(k => {
        return config.corkKubeConfig.gitInfo.httpRemote.includes(k);
      });
      if( match ) {
        localDevRegistry = config.data.build.localDevRegistry[match];
      }
    }
    ...
    let templateVars = Object.assign({}, process.env, {
      ...
      __ENV: opts.env,
      __LOCAL_DEV_REGISTRY: localDevRegistry || config.LOCAL_DEV_REGISTERY
    });
